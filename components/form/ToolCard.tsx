  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-5 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label
            htmlFor={`plan-${toolEntry.id}`}
            className="text-gray-400 text-sm mb-1 block"
          >
            Plan
          </label>
          <select
            id={`plan-${toolEntry.id}`}
        ...
        <div>
          <label
            htmlFor={`seats-${toolEntry.id}`}
            className="text-gray-400 text-sm mb-1 block"
          >
            Seats
          </label>
          <input
            id={`seats-${toolEntry.id}`}
            type="number"
            min="1"
            value={toolEntry.seats}
            onChange={(e) =>
              onUpdate(toolEntry.id, "seats", parseInt(e.target.value) || 1)
            }
            className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm"
          />
        </div>
        ...
        <div>
          <label
            htmlFor={`spend-${toolEntry.id}`}
            className="text-gray-400 text-sm mb-1 block"
          >
            Monthly Spend ($)
          </label>
          <input
            id={`spend-${toolEntry.id}`}
            type="number"
            min="0"
